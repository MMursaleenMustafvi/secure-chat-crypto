...
def encrypt_message(msg, algo, key):
    try:
        if algo == "Caesar": return caesar.encrypt(msg, int(key))
        elif algo == "Affine": 
            a, b = map(int, key.split(","))
            return affine.encrypt(msg, a, b)
        elif algo == "AES": return aes.encrypt(msg, key)
        elif algo == "RailFence": return railfence.encrypt(msg, int(key))
        elif algo == "RowTransposition": return row_transposition.encrypt(msg, key)
        elif algo == "Playfair": return playfair.encrypt(msg, key)
